...
1. **Leaf-sign assignment ambiguity under the sibling-opposite rule.**
   - Paper 1 (Synthetic Datasets section) specifies that sibling leaves must have opposite signs, but does not fully fix whether the assignment is random up to a global sign flip or deterministic by index.
   - `1.DLGN_DLGN_SF_Synthetic.ipynb` uses index-parity-based labels on leaf nodes (`vals = arange(...) % 2` after internal nodes are masked), which implies a deterministic alternation.
   - **Resolution for milestone 1:** deterministic canonical assignment by leaf index parity with optional global sign flip (`global_leaf_sign in {-1,+1}`), default `+1`.

2. **Data sampling for `x` (single_path_dlgn vs multi_path_dlgn vs Paper 1 spec).**
...
3. **Gate activation in the single-path DLGN notebook.**
...
5. **Paper 2's "DLGN" vs Paper 1's taxonomy.**
...
6. **Multi-path training hyperparameter: L2 gating penalty coefficient.**
